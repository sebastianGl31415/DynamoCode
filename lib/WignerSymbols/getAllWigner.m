function [Wigner,Ind]=getAllWigner(j3,m3)
% getAllWigner -- Calculates all Wigner-3j-Symbols for given j3 and m3. The 
% function returns all values and indices [j1 j2 j3 m1 m2 m3] for which the 
% Wigner-3j-Symbols are not vanishing. The indices for j2 and m2 must be stored 
% in the global structure velocity. The computation is performed using recurrence
% relations for the Wigner-3j-symbols.
%
...
%   \ m1  m2  m3 /
%
%             Wigner = getAllWigner(j3,m3) returns the values as a row vector.
%     [Wigner,Index] = getAllWigner(j3,m3) returns the values as a row vector
...
%   See also getWignerZeroM , WignerRecurrence .
	...
	if j3>nmax
		...
		return;
	elseif abs(m3)>j3
		error('2nd input (m3) is larger than 1st input (j3).');
		return
	end
	%% create integer fields to apply selections rules
	[J1,J2,J3,M1,M2,M3]=ndgrid([0:nmax],j2,j3,[-nmax:nmax],m2,m3);
